...
failed=0
wait "$PID_AMD64"   || { echo "ERROR: linux/amd64 build failed"   >&2; failed=1; }
wait "$PID_ARM64"   || { echo "ERROR: linux/arm64 build failed"   >&2; failed=1; }
wait "$PID_ARMV7"   || { echo "ERROR: linux/armv7 build failed"   >&2; failed=1; }
wait "$PID_RISCV64" || { echo "ERROR: linux/riscv64 build failed" >&2; failed=1; }
[ "$failed" -eq 0 ] || exit 1
